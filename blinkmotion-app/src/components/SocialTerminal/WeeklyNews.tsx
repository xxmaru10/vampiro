  ...
  useEffect(() => {
    ...
    if (item?.ascii_url) {
      fetch(item.ascii_url)
        .then(r => r.text())
        .then(t => setAsciiContent(t))
        .catch(() => setAsciiContent(''));
    } else {
      setAsciiContent('');
    }
  }, [currentIndex, news]);
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Header */}
        <div style={{
          background: '#00ff0011', borderBottom: '1px solid #00ff0033',
          padding: '4px 10px', display: 'flex', alignItems: 'center', gap: 10,
          fontSize: '0.8rem', color: '#00ff00cc',
        }}>
          <Newspaper size={14} />
          ...
          <div style={{ display: 'flex', gap: 12, padding: 12 }}>
            {asciiContent && (
              <div style={{
                ...
                width: 110, minWidth: 110, height: 90, overflow: 'hidden',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 4, lineHeight: 1, flexShrink: 0,
              }}>
      ...
      {/* ── CARD DE COMENTÁRIOS (card separado, sempre visível) ── */}
      {currentNews && (
        <div style={{
          background: '#070707',
